...
public class Logic {
    ...
    public Logic() {
        ...
        //----------------------------New Rooms--------------------------------------------
//        rooms.add(new Room("Room1", "Text1a", "Button1-1", "Button1-2", "Button1-3", "room1.jpg"));
...
//        //set inventoty item
//        rooms.get(1).setItem("item0");
//        rooms.get(4).setItem("item1");
//
//        rooms.get(5).setRequiredItemId("item0");
    ...
    public void setActualRoom(Room actualRoom) {
        ...
        if(this.actualRoom.getItem() != null) {
            for (int i = 0; i < this.inventory.length; i++) {
                if(this.inventory[i] != null && inventory[i].equals(this.actualRoom.getItem())) this.inventory[i] = null;
    ...
    public boolean canEnterRoom(Room r) {
        ...
        for (String inventoryItem : inventory) {
            if(r.getRequiredItemId().equals(inventoryItem)) return true;
        }
        return false;
    }
    ...
    public void removeFromInventory(String itemId) {
        ...
        for (int i = 0; i < this.inventory.length; i++) {
            ...
            if(this.inventory[i].equals(itemId)) this.inventory[i] = null;
        }
    }
}